...
async def list_sessions(
    ...
    return [
        SessionResponse(
            ...
            final_output=(json.dumps(s.final_output) if isinstance(s.final_output, (dict, list)) else s.final_output) if hasattr(s, 'final_output') and s.final_output is not None else None,
...
async def get_agent_session_detail(
    ...
    return SessionResponse(
        ...
        loop_count=agent_session.loop_count,
        total_tokens_used=agent_session.total_tokens_used,
        final_output=(json.dumps(agent_session.final_output) if isinstance(agent_session.final_output, (dict, list)) else agent_session.final_output) if hasattr(agent_session, 'final_output') and agent_session.final_output is not None else None,
