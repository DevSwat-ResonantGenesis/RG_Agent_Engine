...
"""
        ...
        # Mirror the generated files into Hash Sphere under the shared
        # project_id so they show up in the IDE's file tree too, not just
        # this service's local build workspace.
        from ..project_builder.gateway_files import push_files_to_gateway
        await push_files_to_gateway(
            project_id, user_id, [{"file_path": f.path, "content": f.content} for f in files]
        )
