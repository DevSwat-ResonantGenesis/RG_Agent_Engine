...
class FrontendProjectRequest(BaseModel):
    ...
    # Shared identifier with the IDE's Hash Sphere storage (RG_Gateway's
    # code_routes.py mints one if the frontend has no project open yet).
    # Lets generated files show up in the IDE's file tree for the same
    # project instead of only living in this service's local build scratch
    # space - see gateway_files.push_files_to_gateway below.
    project_id: Optional[str] = None
...
"""
        ...
        # Mirror the generated files into Hash Sphere under result.project_id
        # - the SAME id returned below as anchors[0]/project_id, which is
        # what the frontend actually tracks as currentProjectId and what
        # the terminal panel seeds from. Previously this pushed under a
        # separate, unrelated id (request.project_id or a fresh uuid4())
        # that was never persisted anywhere else, so Hash Sphere always had
        # zero files under the id the terminal/Claude Code actually looked
        # up - the terminal appeared to never open "inside" the project.
        from ..project_builder.gateway_files import push_files_to_gateway
        await push_files_to_gateway(
            result.project_id, user_id, [{"file_path": f.path, "content": f.content} for f in files]
        )
