...
MEMORY_TOOLS = [
    ...
    ToolDef(name="memory_recall", description="Recall memories using the FULL hash-sphere brain (gravity + associative mesh + cross-encoder rerank + multi-hop fact graph + temporal). Best retrieval; returns confidence.", category=ToolCategory.MEMORY,
            params=[ToolParam("query", ParamType.STRING, "what to recall", required=True), ToolParam("limit", ParamType.INTEGER, "max results", default=8)],
            handler="memory.recall", access={_R, _A}, priority=8, max_result_chars=4000),
    ToolDef(name="memory_facts", description="List distilled atomic facts about the user (name, role, preferences, relationships) — entity-resolved, contradiction-aware.", category=ToolCategory.MEMORY,
            params=[ToolParam("entity", ParamType.STRING, "filter by entity (optional)"), ToolParam("attribute", ParamType.STRING, "filter by attribute (optional)")],
            handler="memory.facts", access={_R, _A}, priority=9, max_result_chars=3000),
...
AGENT_TOOLS_EXTENDED = [
    ...
    ToolDef(name="agents_update", description="Update agent config — name, goal, provider, model, tools, etc.", category=ToolCategory.AGENTS,
            params=[ToolParam("agent_id", ParamType.STRING, "agent UUID", required=True), ToolParam("name", ParamType.STRING), ToolParam("goal", ParamType.STRING), ToolParam("provider", ParamType.STRING, "LLM provider (tokenrouter, openai, anthropic, groq, google, deepseek)"), ToolParam("model", ParamType.STRING), ToolParam("tools", ParamType.ARRAY, items_type="string"), ToolParam("system_prompt", ParamType.STRING), ToolParam("temperature", ParamType.NUMBER), ToolParam("is_active", ParamType.BOOLEAN)],
    ...
    ToolDef(name="run_agent_team", description="Spawn 2-4 specialist agents to work CONCURRENTLY in parallel (e.g. two scriptwriters drafting competing versions at the same time), wait for every one of them to finish, and get all of their finished outputs back together so you can compare and pick or merge the best result instead of committing to a single first attempt.", category=ToolCategory.AGENTS,
            params=[ToolParam("members", ParamType.ARRAY, "2-4 team members to run in parallel, each {agent_id, goal}", required=True)],
            handler="run_agent_team", access={_R, _A}, priority=15),
...
AGENT_ARCHITECT_TOOLS = [
    ...
    ToolDef(name="architect_create_agent", description="Create a fully-configured agent from a blueprint — sets tools, model, provider, system prompt, safety config.",
            ...
            params=[ToolParam("name", ParamType.STRING, "agent name", required=True),
                    ...
                    ToolParam("provider", ParamType.STRING, "LLM provider (tokenrouter, openai, anthropic, groq, google, deepseek)", default="tokenrouter"),
                    ToolParam("model", ParamType.STRING, "model name (e.g. google/gemini-3-flash-preview, anthropic/claude-opus-4.7)", default="google/gemini-3-flash-preview"),
